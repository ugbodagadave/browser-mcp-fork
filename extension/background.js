...
// ── Network Capture (Burp Suite / proxy workflow) ───────────────────────────
// Accumulates CDP Network.requestWillBeSent + responseReceived events per tab.
// Enabled on-demand (via set_proxy or explicit start), cleared on navigation.
// Survives service-worker suspend via chrome.storage checkpointing is NOT done
// here — the log lives in-memory and resets on SW eviction. For the typical
// security-research session (set proxy → navigate → interact → get requests)
// the SW stays alive due to continuous CDP + message activity.

const networkLogs = new Map();      // tabId → array of request entries
const networkCaptureTabs = new Set(); // tabIds with active capture
...
async function ensureNetworkCapture(tabId) {
  ...
  try {
    ...
    console.warn('[MCP] Failed to enable network capture for tab', tabId, e.message);
  }
}
...
function stopNetworkCapture(tabId) {
  networkCaptureTabs.delete(tabId);
  networkLogs.delete(tabId);
}

// Persistent global CDP event listener — filters by tabId in the capture set.
// Multiple onEvent listeners coexist fine (Chrome dispatches to all registered).
chrome.debugger.onEvent.addListener((source, method, eventParams) => {
  if (!source?.tabId || !networkCaptureTabs.has(source.tabId)) return;
  ...
  if (method === 'Network.requestWillBeSent') {
    log.push({
      ...
      method: eventParams.request?.method || 'GET',
      request_headers: eventParams.request?.headers || {},
      timestamp: eventParams.timestamp,
    });
  }

  if (method === 'Network.responseReceived') {
    const entry = log.find(e => e.requestId === eventParams.requestId && !e.status);
    if (entry) {
      entry.status = eventParams.response?.status;
      entry.response_headers = eventParams.response?.headers || {};
      entry.mime_type = eventParams.response?.mimeType || '';
      entry.remote_ip = eventParams.response?.remoteIPAddress || '';
    }
  }

  if (method === 'Network.loadingFinished') {
    const entry = log.find(e => e.requestId === eventParams.requestId && e.status && !e.body);
    if (entry) {
      cdpSend(source.tabId, 'Network.getResponseBody', { requestId: eventParams.requestId })
        .then(bodyResult => {
          if (bodyResult?.body) {
            const body = bodyResult.body;
            entry.body = body.length > 20000 ? body.substring(0, 20000) : body;
            if (body.length > 20000) entry.body_truncated = true;
          }
        })
        .catch(() => {});
    }
...
// Clean up network capture when a tab is closed
chrome.tabs.onRemoved.addListener((tabId) => {
...
async function dispatch(port, method, params) {
  switch (method) {
    case 'navigate': {
      ...
      // Clear network log on navigation (mirrors the old Playwright server behavior)
      clearNetworkLog(tab.id);
      ...
      // Re-enable network capture if it was active before navigation
      // (debugger may have auto-detached during the navigation)
      if (networkCaptureTabs.has(tab.id)) {
        networkCaptureTabs.delete(tab.id); // reset — debuggerAttach will re-add
    ...
    case 'set_proxy': {
      ...
      if (!host || !host.trim()) {
        ...
        return { proxy: 'disabled', message: 'Proxy disabled. Next navigation will be direct.' };
      }

      const proxyHost = `${host.trim()}:${port}`;
      ...
      // Enable network capture on the active tab so get_network_requests works
      const tab = await getSessionTab(port);
      await ensureNetworkCapture(tab.id);

      return {
        proxy: `http://${proxyHost}`,
        message: `Proxy set to ${proxyHost}. Next navigation will route through the proxy.`,
        note: 'Network capture enabled. Use browser_get_network_requests to inspect traffic.',
      };
    }
    ...
    case 'get_network_requests': {
      ...
      // Ensure capture is active (in case set_proxy wasn't called)
      await ensureNetworkCapture(tab.id);

      // Give a brief moment for any in-flight CDP events to land
      await new Promise(r => setTimeout(r, 300));

      let log = networkLogs.get(tab.id) || [];

      // Apply URL filter if provided
      if (urlFilter) {
        log = log.filter(e => e.url && e.url.includes(urlFilter));
      }
      ...
      // Clear the log (default behavior — mirrors old Playwright server)
      if (clear) {
        networkLogs.set(tab.id, []);
      }
